          ...
          <div class="course-info">
            ...
            <div class="info-columns">
              ...
              <div class="info-column">
                ...
                <div class="info-item"><i class="el-icon-user-solid"></i> 学习人数：{{ course.subscriber_count || 0 }}</div>
              </div>
            </div>
            ...
            <div class="course-actions">
              <el-button
                  class="course-enter-button"
                  size="default"
                  @click.stop="viewCourseContent(course.course_id)"
              >进入课堂</el-button>
              <el-button
                  class="course-delete-button"
                  size="default"
                  type="danger"
                  @click.stop="cancelSubscription(course.course_id)"
              >取消订阅</el-button>
            </div>
          </div>
        </div>
...
// 获取订阅课程列表
const fetchSubscribedCourses = debounce(async () => {
  ...
  try {
    ...
    const response = await axiosInstance.get('/api/student/get-subscribed-course', {
      params: { student_id: id }
    })

    if (response.data.success) {
      courses.value = response.data.data.courses || []
      totalCourses.value = response.data.data.total || 0
    } else {
      ElMessage.error(response.data.message || '获取订阅课程失败')
    }
  } catch (error) {
    console.error('获取订阅课程失败', error)
    ElMessage.error('获取订阅课程失败，请检查网络连接')
  } finally {
    isLoading.value = false
  }
}, 300)
...
const viewCourseContent = (courseId) => {
  router.push({
    ...
    ElMessage.error('跳转课程内容失败，请重试')
  })
}

// 取消订阅
const cancelSubscription = async (courseId) => {
  try {
    // 确认对话框
    await ElMessageBox.confirm(
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          const response = await axiosInstance.delete('/api/student/cancel-subscribe', {
            data: formData,
          ...
          if (response.data.success) {
            ...
            fetchSubscribedCourses() // 刷新课程列表
            resolve()
          } else {
            ElMessage.error(response.data.message || '取消订阅失败')
            reject(response.data.message || '取消订阅失败')
          }
        } catch (error) {
          console.error('取消订阅失败', error)
          if (error.response) {
            ElMessage.error(error.response.data.message || '取消订阅失败')
          } else {
      ...
      // 捕获取消操作，区分是用户取消还是实际错误
      if (err === 'cancel') {
        ElMessage.info('中止操作')
      } else {
        console.error('取消订阅过程出错', err)
      }
    })
  } catch (error) {
    console.error('取消订阅函数异常', error)
  }
}
